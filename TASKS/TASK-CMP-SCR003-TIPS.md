...
`TASKS/00_TASK_LIST.md`(Seq 37, Category CMP)에 정의된 Task를 실제로 개발 가능한 단위로 구체화한 문서다. 이 Task는 "찾기 Tip 3개"을(를) 다룬다. 대상 Screen은 통합 여행 준비(`/travel-tools`)이며 Route는 `/travel-tools`다. 기반 문서: `docs/06_SRS_UIUX_REVISED.md`(요구사항 원문), `docs/PROJECT_SCOPE.md`(구현 범위 판단), `design-reference/D-001/DESIGN.md`·`design-reference/UI_CONTRACT.md`(디자인 계약), `design-reference/SCREEN_ROUTE_CONTRACT.json`(Route/Page Entry 정본).

## Project Scope

`docs/PROJECT_SCOPE.md`의 요구사항 분류 기준으로 이 Task의 Implementation Status는 **IMPLEMENT**다. 이 Task에 결부된 Requirement(구조적(비전달 고지 연계 REQ-FUNC-015/023/054))는 EXCLUDED가 아니므로 구현 대상이며, `TASKS/00_TASK_LIST.md` §8 NON_IMPLEMENTATION Register에는 등장하지 않는다.
...
- 신규 생성: `src/app/_components/scr003/tips.tsx`
- 위 목록 밖의 파일은 이 Task 범위에서 수정하지 않는다.

## Functional AC

- [ ] 항공·숙소 찾기 실용 팁 3개 카드+비전달 고지 배너
...
- [ ] 연결된 Requirement(REQ-FUNC-015)가 `docs/UIUX_TRACEABILITY.md` 상태와 모순되지 않는다.
- [ ] Test Cases(TC-FUNC-054)가 통과한다.
