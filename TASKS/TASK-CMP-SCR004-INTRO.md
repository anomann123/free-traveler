...
`TASKS/00_TASK_LIST.md`(Seq 41, Category CMP)에 정의된 Task를 실제로 개발 가능한 단위로 구체화한 문서다. 이 Task는 "동행 찾기 Intro"을(를) 다룬다. 대상 Screen은 동행 조회(`/mates`)이며 Route는 `/mates`다. 기반 문서: `docs/06_SRS_UIUX_REVISED.md`(요구사항 원문), `docs/PROJECT_SCOPE.md`(구현 범위 판단), `design-reference/D-001/DESIGN.md`·`design-reference/UI_CONTRACT.md`(디자인 계약), `design-reference/SCREEN_ROUTE_CONTRACT.json`(Route/Page Entry 정본).
...
- 신규 생성: `src/app/_components/scr004/intro.tsx`
- 위 목록 밖의 파일은 이 Task 범위에서 수정하지 않는다.

## Functional AC

- [ ] 한 문장 소개+"새 동행글 작성" CTA(`/travel-tools` 이동)
