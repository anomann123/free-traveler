# E2E-MATE-AUTH — 동행·계정 Smoke(SCR-004/005)
...
`TASKS/00_TASK_LIST.md`(Seq 61, Category E2E)에 정의된 Task를 실제로 개발 가능한 단위로 구체화한 문서다. 이 Task는 "동행·계정 Smoke(SCR-004/005)"을(를) 다룬다. 특정 Screen에 종속되지 않는 데이터/인프라/검증 계층 Task다. 기반 문서: `docs/06_SRS_UIUX_REVISED.md`(요구사항 원문), `docs/PROJECT_SCOPE.md`(구현 범위 판단), `design-reference/D-001/DESIGN.md`·`design-reference/UI_CONTRACT.md`(디자인 계약), `design-reference/SCREEN_ROUTE_CONTRACT.json`(Route/Page Entry 정본).
...
- 신규 생성: `tests/e2e/mate-auth.spec.ts`
- 위 목록 밖의 파일은 이 Task 범위에서 수정하지 않는다.

## Functional AC

- [ ] 흐름 1) 로그인→동행 목록 필터→상세→참가 요청 제출 2) `/account`에서 참가 요청 승인/거절 3) 관리자 계정으로 신고 상태 변경+외부 URL 저장. **Chromium 전용 Smoke**
